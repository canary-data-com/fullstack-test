defmodule InsiderTradingInfo.Trading do
  @moduledoc """
  The Trading context is to fetch information of different entity from the database
  """

  import Ecto.Query, warn: false
  alias InsiderTradingInfo.Repo

  alias InsiderTradingInfo.InsiderTrade
  alias InsiderTradingInfo.CompanyInfo
  ...
  def company_for_previous_years_data_fetched() do
    CompanyInfo
    |> where([ci], ci.previous_years_data_fetched == false)
    |> limit(1)
    |> Repo.one()
    |> Repo.preload(insider_trades: from(it in InsiderTrade, order_by: [asc: it.filling_date]))
  end

  def company_list_for_recent_data_fetched() do
    CompanyInfo
    |> where([ci], ci.previous_years_data_fetched == true)
    |> Repo.all()
    |> Repo.preload(insider_trades: from(it in InsiderTrade, order_by: [asc: it.filling_date]))
  end
